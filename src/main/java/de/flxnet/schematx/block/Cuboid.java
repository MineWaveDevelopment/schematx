...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
	...
	public List<SchemaBlockDescription> getSchemaBlockDescriptionList() {
		return getBlocks().stream().map(block -> new SchemaBlockDescription(block, this.startLocation)).collect(Collectors.toList());
	}
	
	public List<BlockData> getBlockDataList() {
		return getBlocks().stream().map(Block::getBlockData).collect(Collectors.toList());
	}
	...
	public List<Block> getBlocks() {
		...
		while (blockI.hasNext()) copy.add((Block) blockI.next());
		return copy;
	}
